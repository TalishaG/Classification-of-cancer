...
```{r}
options(timeout = 600)
gse_set <- getGEO("GSE14520", GSEMatrix = TRUE)

```

```{r}
gse <- gse_set[["GSE14520-GPL3921_series_matrix.txt.gz"]]
...
# build label factor (ensure exact levels)
if (!("characteristics_ch1" %in% colnames(pheno))) stop("characteristics_ch1 not found; inspect pData(gse)")
labels <- factor(ifelse(grepl("^Tissue: Liver Tumor Tissue$", pheno$characteristics_ch1, ignore.case = TRUE),
...
meta_features_all <- function(X_all,
  ...
  library(factoextra)
  scree_plot<-fviz_eig(gpca, addlabels = TRUE, ylim = c(0, 90), 
...
MF <- meta_features_all(X_all, top_var = 7000, pcs_for_kmeans = 30, K = 60, seed = 2000)
ranks <- rank_features(MF$all, y_all)
selected <- head(ranks$feature, n = min(5, ncol(MF$all)))
print(selected)
...
```{r}
# ------------------------------
# Feature Selection: PLS & Entropy on raw data
# ------------------------------

# --- Reduce dimensionality for PLS ---
top_var <- 7000  # adjust if needed
v <- apply(X_all, 2, var, na.rm = TRUE)
keep_genes <- names(sort(v, decreasing = TRUE))[1:min(top_var, ncol(X_all))]
X_filtered <- X_all[, keep_genes, drop = FALSE]

# --- Entropy selection ---
ranks_entropy <- rank_features(as.data.frame(X_filtered), y_all)
top_entropy <- head(ranks_entropy$feature, 5)

# --- PLS selection ---
pls_model_raw <- plsr(as.numeric(y_all) - 1 ~ ., data = as.data.frame(X_filtered), ncomp = 6, validation = "CV")
pls_loadings <- abs(loadings(pls_model_raw))
pls_top_features <- colnames(X_filtered)[order(rowSums(pls_loadings), decreasing = TRUE)[1:6]]

# --- Feature sets ---
feature_sets_raw <- list(
  Entropy = top_entropy,
  PLS     = pls_top_features
)
...
for (i in seq_along(folds)) {
  ...
  for (fs_name in names(feature_sets_raw)) {
    selected_features <- feature_sets_raw[[fs_name]]
    
    X_train <- as.matrix(Xtr[, selected_features, drop = FALSE])
    X_test  <- as.matrix(Xte[, selected_features, drop = FALSE])
    
    # --- Preserve correct column names for HMM ---
    X_train_df <- data.frame(Class = ytr, X_train, check.names = FALSE)
    colnames(X_train_df)[-1] <- selected_features
    X_test_df <- data.frame(Class = yte, X_test, check.names = FALSE)
    colnames(X_test_df)[-1] <- selected_features
    
    # --- Train caret models ---
    models <- list()
    models$logistic <- fit_glmnet(X_train, ytr, inner_trctrl)
    models$rf       <- caret::train(x = X_train, y = ytr, method = "rf", metric = "ROC",
                                    trControl = inner_trctrl, tuneLength = 5, ntree = 500)
    models$knn      <- caret::train(x = X_train, y = ytr, method = "knn", metric = "ROC",
                                    trControl = inner_trctrl, tuneLength = 5)
    models$svm      <- caret::train(x = X_train, y = ytr, method = "svmRadial", metric = "ROC",
                                    trControl = inner_trctrl, tuneLength = 5)
    models$xgb      <- caret::train(x = X_train, y = ytr, method = "xgbTree", metric = "ROC",
                                    trControl = inner_trctrl, tuneLength = 5,
                                    tuneGrid = expand.grid(nrounds = 100, max_depth = 3, eta = 0.1,
                                                           gamma = 0, colsample_bytree = 0.8,
                                                           min_child_weight = 1, subsample = 0.8))
    models$bayesglm <- caret::train(x = X_train, y = ytr, method = "bayesglm", metric = "ROC",
                                    trControl = inner_trctrl)
    ...
    # --- HMM ---
n_tumor <- sum(ytr == "Tumor")
n_normal <- sum(ytr == "Normal")
prior_ratio <- log(n_tumor / n_normal)

# Set separate states/bins for PLS vs Entropy
if (fs_name == "PLS") {
  states_to_try <- 4:5    # for example
  bins_to_try   <- 3:4
} else {
  states_to_try <- 3:4     # keep original for Entropy
  bins_to_try   <- 3:5
}

hmm_res <- tune_hmm(train_df = X_train_df,
                    test_df  = X_test_df,
                    selected_features = selected_features,
                    prior_ratio = prior_ratio,
                    states_list = states_to_try,
                    bins_list   = bins_to_try)

# --- Store HMM results ---
probs_list[[paste(fs_name, "HMM", sep = "_")]] <- hmm_res$probs
if (is.null(best_hmms_raw[[fs_name]])) best_hmms_raw[[fs_name]] <- list()
best_hmms_raw[[fs_name]]$tumor <- hmm_res$tumor
best_hmms_raw[[fs_name]]$normal <- hmm_res$normal
  ...
  fold_results_raw[[i]] <- models_metrics %>% mutate(fold = i)
}

# --- Aggregate CV results ---
cv_results_raw <- bind_rows(fold_results_raw)
cv_summary_raw <- cv_results_raw %>%
  group_by(model) %>%
...
Now we include the full PKPB Meta feature pipeline, to compare results:
...
for (i in seq_along(folds)) {
  ...
  pls_model <- plsr(as.numeric(ytr) - 1 ~ ., data = Etr_df[, -1, drop = FALSE], ncomp = 5, validation = "CV")
  pls_loadings <- abs(loadings(pls_model))
  pls_top_features <- colnames(Etr_df)[-1][order(rowSums(pls_loadings), decreasing = TRUE)[1:min(5, ncol(Etr_df)-1)]]
  ...
  for (fs_name in names(feature_sets)) {
    ...
    hmm_res <- tune_hmm(train_df = fs$train, test_df = fs$test, selected_features = selected_features,
                        prior_ratio = prior_ratio, states_list = 3:4, bins_list = 3:5)
...
```{r}
# -------------------------------
# Normal 80/20 Split (no CV)
# -------------------------------
set.seed(123)
split123_idx <- createDataPartition(y_all, p = 0.8, list = FALSE)
train123_idx <- split123_idx
test123_idx  <- setdiff(seq_len(nrow(X_all)), train123_idx)

Xtr123 <- X_all[train123_idx, , drop = FALSE]
Xte123 <- X_all[test123_idx, , drop = FALSE]
ytr123 <- factor(y_all[train123_idx], levels = c("Tumor", "Normal"))
yte123 <- factor(y_all[test123_idx], levels = c("Tumor", "Normal"))

Etr123 <- E_all_df[train123_idx, ]
Ete123 <- E_all_df[test123_idx, ]

# -------------------------------
# PLS using PKPB meta-features
# -------------------------------
pls123_model <- plsr(as.numeric(ytr123) - 1 ~ ., data = Etr123[, -1, drop = FALSE],
                     ncomp = 5, validation = "none")  # no CV
pls123_loadings <- abs(loadings(pls123_model))
pls123_top_features <- colnames(Etr123)[-1][
  order(rowSums(pls123_loadings), decreasing = TRUE)[1:min(5, ncol(Etr123)-1)]
]

# -------------------------------
# Feature sets
# -------------------------------
feature_sets123 <- list(
  Entropy = list(train = Etr123, test = Ete123, selected = selected),
  PLS = list(
    train = data.frame(Etr123[, pls123_top_features, drop = FALSE], Class = ytr123),
    test  = data.frame(Ete123[, pls123_top_features, drop = FALSE], Class = yte123),
    selected = pls123_top_features
  )
)
...
for (fs_name in names(feature_sets123)) {
  fs <- feature_sets123[[fs_name]]
  selected_features <- fs$selected %||% colnames(fs$train)[colnames(fs$train) != "Class"]
  ...
  # -------------------------------
  # Fit caret models WITHOUT CV
  # -------------------------------
  models123 <- list()
  no_cv_ctrl <- trainControl(method = "none", classProbs = TRUE, summaryFunction = twoClassSummary)
  
  models123$logistic <- caret::train(
    x = X_train, y = y_train,
    method = "glmnet",
    trControl = no_cv_ctrl,
    tuneLength = 1,
    metric = "ROC"
  )
  
  models123$rf <- caret::train(
    x = X_train, y = y_train,
    method = "rf",
    trControl = no_cv_ctrl,
    metric = "ROC",
  ...
  models123$bayesglm <- caret::train(
    x = X_train, y = y_train,
    method = "bayesglm",
    trControl = no_cv_ctrl,
    metric = "ROC"
  ...
  # Predictions
  # -------------------------------
  for (nm in names(models123)) {
    probs123_list[[paste(fs_name, nm, sep = "_")]] <- get_prob(models123[[nm]], as.data.frame(X_test))
  }
  ...
  probs123_list[[paste(fs_name, "HMM", sep = "_")]] <- hmm123_res$probs
}

# -------------------------------
# Evaluate performance
# -------------------------------
split123_metrics <- tibble()
for (mname in names(probs123_list)) {
  pr <- probs123_list[[mname]]
  if (all(is.na(pr))) {
    row <- tibble(model = mname, AUC = NA_real_, Accuracy = NA_real_,
                  Sensitivity = NA_real_, Specificity = NA_real_,
                  F1 = NA_real_, FN = NA_real_)
  } else {
    pred_class <- factor(ifelse(pr >= 0.5, "Tumor", "Normal"), levels = levels(yte123))
    auc_val <- tryCatch(as.numeric(pROC::roc(response = yte123, predictor = pr, quiet = TRUE)$auc),
                        error = function(e) NA_real_)
    cm <- tryCatch(confusionMatrix(pred_class, yte123, positive = "Tumor"), error = function(e) NULL)
    if (is.null(cm)) {
      row <- tibble(model = mname, AUC = auc_val, Accuracy = NA_real_,
                    Sensitivity = NA_real_, Specificity = NA_real_,
                    F1 = NA_real_, FN = NA_real_)
    } else {
      row <- tibble(model = mname, Accuracy = cm$overall["Accuracy"], AUC = auc_val,
  ...
  split123_metrics <- bind_rows(split123_metrics, row)
}

split123_summary <- split123_metrics %>%
  arrange(desc(Accuracy), desc(Sensitivity), desc(AUC))
...
```{r}
#5 Fold CV under set.seed(555)
outer_folds555 <- 5
set.seed(555)
folds555 <- createFolds(y_all, k = outer_folds555, list = TRUE, returnTrain = FALSE)
fold_results555 <- list()
best_hmms555 <- list()

for (i in seq_along(folds555)) {
  cat("Running outer fold (seed 555):", i, "\n")
  
  test555_idx <- folds555[[i]]
  train555_idx <- setdiff(seq_len(nrow(X_all)), test555_idx)
  
  Xtr555 <- X_all[train555_idx, , drop = FALSE]
  Xte555 <- X_all[test555_idx, , drop = FALSE]
  ytr555 <- factor(droplevels(y_all[train555_idx]), levels = c("Tumor", "Normal"))
  yte555 <- factor(droplevels(y_all[test555_idx]), levels = c("Tumor", "Normal"))
  
  Etr555 <- E_all_df[train555_idx, ]
  Ete555 <- E_all_df[test555_idx, ]
  
  pls555_model <- plsr(as.numeric(ytr555) - 1 ~ ., data = Etr555[, -1, drop = FALSE], 
                       ncomp = 5, validation = "CV")
  pls555_loadings <- abs(loadings(pls555_model))
  pls555_top_features <- colnames(Etr555)[-1][order(rowSums(pls555_loadings),
                                                    decreasing = TRUE)[1:min(5, ncol(Etr555)-1)]]
  
  feature_sets555 <- list(
    Entropy = list(train = Etr555, test = Ete555, selected = selected),
    PLS = list(
      train = data.frame(Etr555[, pls555_top_features, drop = FALSE], Class = ytr555),
      test  = data.frame(Ete555[, pls555_top_features, drop = FALSE], Class = yte555),
      selected = pls555_top_features
    )
  )
  
  inner_trctrl555 <- make_inner_trctrl(seed = 5000 + i)
  probs555_list <- list()
  
  for (fs_name in names(feature_sets555)) {
    fs <- feature_sets555[[fs_name]]
    selected_features <- fs$selected %||% colnames(fs$train)[colnames(fs$train) != "Class"]
    ...
    models555 <- list()
    models555$logistic <- fit_glmnet(X_train, y_train, inner_trctrl555)
    models555$rf <- caret::train(x = X_train, y = y_train, method = "rf", metric = "ROC",
                                 trControl = inner_trctrl555, tuneLength = 5, ntree = 500)
    models555$knn <- caret::train(x = X_train, y = y_train, method = "knn", metric = "ROC",
                                  trControl = inner_trctrl555, tuneLength = 5)
    models555$svm <- caret::train(x = X_train, y = y_train, method = "svmRadial", metric = "ROC",
                                  trControl = inner_trctrl555, tuneLength = 5)
    models555$xgb <- caret::train(x = X_train, y = y_train, method = "xgbTree", metric = "ROC",
                                  trControl = inner_trctrl555, tuneLength = 5,
                                  tuneGrid = expand.grid(nrounds = 100, max_depth = 3, eta = 0.1,
                                                         gamma = 0, colsample_bytree = 0.8,
                                                         min_child_weight = 1, subsample = 0.8))
    models555$bayesglm <- caret::train(x = X_train, y = y_train, method = "bayesglm",
                                       metric = "ROC", trControl = inner_trctrl555)
    
    for (nm in names(models555)) {
      probs555_list[[paste(fs_name, nm, sep = "_")]] <- get_prob(models555[[nm]], as.data.frame(X_test))
    }
    
    n_tumor <- sum(y_train == "Tumor")
    n_normal <- sum(y_train == "Normal")
    prior_ratio <- log(n_tumor / n_normal)
    hmm555_res <- tune_hmm(train_df = fs$train, test_df = fs$test,
                           selected_features = selected_features,
                           prior_ratio = prior_ratio, states_list = 3:4, bins_list = 3:5)
    
    probs555_list[[paste(fs_name, "HMM", sep = "_")]] <- hmm555_res$probs
    if (is.null(best_hmms555[[fs_name]])) best_hmms555[[fs_name]] <- list()
    best_hmms555[[fs_name]]$tumor <- hmm555_res$tumor
    best_hmms555[[fs_name]]$normal <- hmm555_res$normal
  }
  
  models_metrics555 <- tibble()
  for (mname in names(probs555_list)) {
    pr <- probs555_list[[mname]]
    if (all(is.na(pr))) {
      row <- tibble(model = mname, AUC = NA_real_, Accuracy = NA_real_, Sensitivity = NA_real_,
                    Specificity = NA_real_, F1 = NA_real_, FN = NA_real_)
    } else {
      pred_class <- factor(ifelse(pr >= 0.5, "Tumor", "Normal"), levels = levels(yte555))
      auc_val <- tryCatch(as.numeric(pROC::roc(response = yte555, predictor = pr, quiet = TRUE)$auc),
                          error = function(e) NA_real_)
      cm <- tryCatch(confusionMatrix(pred_class, yte555, positive = "Tumor"), error = function(e) NULL)
      if (is.null(cm)) {
        row <- tibble(model = mname, AUC = auc_val, Accuracy = NA_real_, Sensitivity = NA_real_,
                      Specificity = NA_real_, F1 = NA_real_, FN = NA_real_)
      } else {
    ...
    models_metrics555 <- bind_rows(models_metrics555, row)
  }
  fold_results555[[i]] <- models_metrics555 %>% mutate(fold = i)
}

cv_results555 <- bind_rows(fold_results555)
cv_summary555 <- cv_results555 %>%
  group_by(model) %>%
  summarise(Accuracy = mean(Accuracy, na.rm = TRUE),
            AUC = mean(AUC, na.rm = TRUE),
            Sensitivity = mean(Sensitivity, na.rm = TRUE),
...
# Normal 80/20 Split under set.seed(555) (no CV)

set.seed(555)
split555_idx <- createDataPartition(y_all, p = 0.8, list = FALSE)
train555_idx <- split555_idx
test555_idx  <- setdiff(seq_len(nrow(X_all)), train555_idx)

Xtr555 <- X_all[train555_idx, , drop = FALSE]
Xte555 <- X_all[test555_idx, , drop = FALSE]
ytr555 <- factor(y_all[train555_idx], levels = c("Tumor", "Normal"))
yte555 <- factor(y_all[test555_idx], levels = c("Tumor", "Normal"))
...
pls555_model <- plsr(as.numeric(ytr555) - 1 ~ ., data = Etr555[, -1, drop = FALSE],
                     ncomp = 5, validation = "none")  # no CV
pls555_loadings <- abs(loadings(pls555_model))
pls555_top_features <- colnames(Etr555)[-1][
  order(rowSums(pls555_loadings), decreasing = TRUE)[1:min(5, ncol(Etr555)-1)]
]
...
for (fs_name in names(feature_sets555)) {
  ...
  models555$logistic <- caret::train(
    x = X_train, y = y_train,
    method = "glmnet",
    trControl = no_cv_ctrl,
    tuneLength = 1,
  ...
  models555$bayesglm <- caret::train(
    x = X_train, y = y_train,
    method = "bayesglm",
    trControl = no_cv_ctrl,
    metric = "ROC"
...
for (mname in names(probs555_list)) {
  ...
  split555_metrics <- bind_rows(split555_metrics, row)
}

split555_summary <- split555_metrics %>%
  arrange(desc(Accuracy), desc(Sensitivity), desc(AUC))
...
# ---- Function to Save + Display Combined Plots ----
save_hmm_plots <- function(plot_list, filename, ncol = 2, tile_scale = 1.2) {
  combined <- cowplot::plot_grid(plotlist = plot_list, ncol = ncol, align = "hv")
  print(combined)  # show in RStudio
  ggsave(filename, plot = combined, width = 12, height = 8, dpi = 300)
  return(combined)
}
...
emission_combined <- save_hmm_plots(emission_plots, "GSE14520_emissions_plot.png", ncol = 2)
...
transition_combined <- save_hmm_plots(transition_plots, "GSE14520_transitions_plot.png", ncol = 2)
...
pca_plot <- ggplot(plot_data, aes(x = PC1, y = PC2, color = Class, shape = Class)) +
  ...
  labs(
    title = "PCA of Selected Meta-Features for the Liver Cancer Dataset",
