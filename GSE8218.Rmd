...
gse <- getGEO("GSE8218", GSEMatrix = TRUE)[[1]]
```
...
# Build labels for GSE8218
labels <- factor(
  ifelse(!grepl("Percentage of Tumor: 0%", pheno$characteristics_ch1),
         "Tumor", "Normal"),
...
MF <- meta_features_all(X_all, top_var = 3000, pcs_for_kmeans = 25, K = 50, seed = 2000)
ranks <- rank_features(MF$all, y_all)
selected <- head(ranks$feature, n = min(3, ncol(MF$all)))
print(selected)
...
for (i in seq_along(folds)) {
  ...
  pls_model <- plsr(as.numeric(ytr) - 1 ~ ., data = Etr_df[, -1, drop = FALSE], ncomp = 3, validation = "CV")
  pls_loadings <- abs(loadings(pls_model))
  pls_top_features <- colnames(Etr_df)[-1][order(rowSums(pls_loadings), decreasing = TRUE)[1:min(3, ncol(Etr_df)-1)]]
  ...
  for (fs_name in names(feature_sets)) {
    ...
    hmm_res <- tune_hmm(train_df = fs$train, test_df = fs$test, selected_features = selected_features,
                        prior_ratio = prior_ratio, states_list = 3:4, bins_list = 3:4)
